...
误差。单发先前进到“整发步距+8%弹位”，稳定后反转回严格的整发落点，
避免停在出弹临界位置；8%只相当于输出轴`2.88°`，远小于下一枚弹丸的
`36°`间距。拨轮快速回中也会完成已经触发的这一发。连发为`3000 rpm`，
退弹为保守的`-300 rpm`，单发最大`600 rpm`。PID、三种动作限流、拨轮
阈值、超转比例和单发位置参数全部在`Core/Inc/config/control_tuning.h`。
...
TGT=目标转速×100,POS=多圈编码器,REF=当前单发阶段目标,PE=位置误差,
SHOT=完成单发数,SA=单发运行中,SR=单发回退阶段,E=速度误差,
P=比例项,I=积分项,D=微分项,
